...
This boilerplate creates a working Pulumi-owned provider named `railway`.
...
1. Set the following options:
   * Owner: pulumi 
   * Repository name: pulumi-railway (replace "railway" with the name of your provider)
   * Description: Pulumi provider for railway
...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `railway` with the `NAME` of your provider.
   - find and replace all instances of the boilerplate `roboautomator` with the `ORG` of your provider.
   - replace all instances of the `github.com/roboautomator/pulumi-railway` repository with the `REPOSITORY` location
...
1. A `provider/` folder containing the building and implementation logic
    1. `cmd/pulumi-resource-railway/main.go` - holds the provider's sample implementation logic.
2. `deployment-templates` - a set of files to help you around deployment and publication
3. `sdk` - holds the generated code libraries created by `pulumi-gen-railway/main.go`
